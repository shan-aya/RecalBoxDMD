...
- **PC Toolkit**: corregida la desaparición completa de la parte inferior de la interfaz (la sección de Progreso) en pantallas de alta resolución (reportado por un usuario en una pantalla 4K al 125% de escala) — el tamaño de la ventana estaba fijado en píxeles brutos mientras las fuentes se escalaban con el DPI del sistema, así que a partir de cierta escala el contenido dejaba de caber en la ventana; la escala interna de la aplicación ahora está fijada a la base del 100% de DPI con la que siempre se ha probado, sea cual sea el ajuste real de DPI del sistema.
