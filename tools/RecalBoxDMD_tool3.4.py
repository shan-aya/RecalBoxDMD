...
TRANSLATIONS = {
    "fr": {
        ...
        "mode1_title": "MODE 1 — Extraction + Conversion raw 128x32 + Build Cache",
...
def mode_full(sd_dir: Path):
        ...
        for f in Path(systems_out).rglob(ext):
            ...
            if raw565_path.exists():
                try:
                    ...
                    removed_errors += 1
    print(f"\n🗑️  {removed_files} fichiers .png/.gif supprimés (remplacés par raw565)")
    if removed_errors:
        print(f"⚠️  {removed_errors} fichiers .png/.gif n'ont pas pu être supprimés")
    ...
    if conv_errors > 0:
        print(f"\n⚠️  {conv_errors} erreur(s) de conversion (voir détails ci-dessus)")
    print(f"\n   {tr('done_copy_sd')}")
