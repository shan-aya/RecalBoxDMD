...
- **PC Toolkit** : corrige la disparition complète du bas de l'interface (la section Progression) sur les écrans haute résolution (signalé par un testeur sur un écran 4K à 125 % de mise à l'échelle) — la taille de la fenêtre était fixée en pixels bruts alors que les polices s'agrandissaient avec le DPI système, si bien qu'au-delà d'un certain seuil le contenu ne tenait plus dans la fenêtre ; l'échelle interne de l'application est désormais verrouillée sur la base 100 % DPI avec laquelle elle a toujours été testée, quel que soit le réglage DPI réel du système.
