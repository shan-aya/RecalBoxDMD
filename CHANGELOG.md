...
- **PC Toolkit**: fixed the bottom of the interface (the Progress section) disappearing entirely on high-DPI displays (reported by a tester on a 4K screen at 125% scaling) — the window size was fixed in raw pixels while fonts scaled up with the system's DPI setting, so past a certain scale the content no longer fit inside the window; the app's internal scale is now locked to the 100%-DPI baseline it's always been tested at, regardless of the system's actual DPI setting.
